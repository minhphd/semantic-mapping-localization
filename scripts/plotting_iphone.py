"""
Stray Scanner Dataset Visualization and Reconstruction Utility
===============================================================

This script processes and visualizes RGB-D datasets collected using the
Stray Scanner mobile application. It supports trajectory visualization,
camera frame visualization, point cloud generation, and volumetric
TSDF-based RGB-D integration using Open3D.

The dataset directory is expected to contain:

    <dataset_root>/
    ├── camera_matrix.csv      # 3x3 intrinsic calibration matrix
    ├── odometry.csv           # timestamped poses (position + quaternion)
    ├── rgb.mp4                # RGB video stream
    ├── depth/                 # Per-frame depth maps (.npy or .png, in mm)
    └── confidence/            # Per-frame confidence maps (.png)

Core Features
-------------
• Convert per-frame depth maps into global point clouds  
• Integrate RGB-D frames into a TSDF volume  
• Export reconstructed point clouds (.ply)  
• Visualize trajectory and camera coordinate frames  

Coordinate Conventions
----------------------
• Poses in odometry.csv are world-to-camera transforms (T_WC)
• Depth is stored in millimeters and converted internally to meters
• RGB frames are resized to match depth resolution (256x192)
• Intrinsics are scaled accordingly to match depth dimensions

Typical Usage
-------------
Basic visualization:
    python stray_visualize.py <dataset_path>

Export merged point cloud:
    python stray_visualize.py <dataset_path> --point-clouds

Integrate TSDF and save mesh:
    python stray_visualize.py <dataset_path> --integrate --mesh-filename output.ply

Important Parameters
--------------------
--every         Process every Nth frame (default: 5)
--voxel-size    TSDF voxel resolution in meters (default: 0.015)
--confidence    Minimum depth confidence threshold (0–2)
...
Author
------
Designed for processing iPhone-based RGB-D scans collected via Stray Scanner.
"""
...
def main():
    ...
    if flags.point_clouds:
        ...
        o3d.io.write_point_cloud("millerst.ply", pc[0])
    # if flags.integrate:
